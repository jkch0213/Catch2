...
public class Login {
    ...
    public static String LoadInfo(String id) throws SQLException{
	    ...
	    try {
	    	stmt = (PreparedStatement) conn.prepareStatement(
	    			"SELECT DISTINCT level,exp,coin,avatar,numMoreExp,numMoreCoin,numShowOneword,numhowmanyword FROM info inner join item WHERE info.user_id = ?");
	    	stmt.setString(1, id);
	    ...
	    if((ID.equals(id) ))	return LEVEL +"\t"+EXP+"\t"+COIN+"\t"+AVATAR+"\t"+numMoreExp+"\t"
	    				+numMoreCoin+"\t"+numShowOneword+"\t"+numHowmanyword;
    	else return "false";
